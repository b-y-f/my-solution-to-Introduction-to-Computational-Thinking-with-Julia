...
function mean(v)
	...
	return sum([i for i in v])/length(v)
end
...
function box_blur_kernel(l)
	...
	return [1/(2*l+1) for i in range(1,length=2*l+1)]
	
end

# ╔═╡ a39d0598-add2-4ffb-b614-7ed9a8aa5bec
Kernel.gaussian((1,))
...
# ╔═╡ 3ea44917-b2bc-4e9e-bad1-5fdf5b7b3c35
Kernel.gaussian((4,))
...
@bind gaussian_kernel_size_1D Slider(0:6,show_value=true)
...
function box_blur(v::AbstractArray, l)
	...
	return [mean([extend(v,j) for j in range(i-l,i+l,step=1)]) for i in range(1,length=length(v))]
			
end

# ╔═╡ 68368f96-7af7-4e89-a5f4-2d48aa598100
...
# ╔═╡ 7b18f88d-cba8-4fa5-9413-30ab35b3025a
box_blur(v,l_box)
...
function convolve(v::AbstractVector, k)
	...
	# k = OffsetArray(k, OffsetArrays.Origin(1))

	return [sum([extend(v,j) for j in range(i-l,i+l,step=1)] .* k) for i in range(1,length=length(v))]
...
function convolve(M::AbstractMatrix, K::AbstractMatrix)
	...
	center_r = (size(K,1)-1)÷2
	center_c = (size(K,2)-1)÷2
		
	result =zeros(RGB,size(M,1),size(M,2))
	
	for y in 1:size(M,1)
		for x in 1:size(M,2)
			
			result[y,x] = sum([extend(M,i,j) for
				i in y - center_r:y+center_r,
				j in x - center_c:x+center_c
				] .* K)
...
# ╔═╡ f0c10bb0-fb62-4450-8f70-0b5e33937199
test_convolution2 = let
	k = [0.005, 0.1,0.4,0.1,0.005]
	convolve(v, k)
end

# ╔═╡ 5eea882c-ee13-11ea-0d56-af81ecd30a4a
colored_line(test_convolution2)
...
# ╔═╡ 0bc17a54-8798-4474-b910-01d6dc6de553
sum(K_test)
...
function gaussian_kernel_1D(n; σ=1)
	...
	g = [gauss(x, σ=σ) for x in -n:n]
	
	return g ./ sum(g)
end

# ╔═╡ d4209f39-3760-4f6e-84e6-65ba5d4575f5
gaussian_kernel_1D(4; σ=2)

# ╔═╡ a6149507-d5ba-45c1-896a-3487070d36ec
colored_line(gaussian_kernel_1D(1; σ=1))
...
# ╔═╡ b1b07a14-d5e0-4162-946f-b4ebdfc8c761
# g = [gauss(x, σ=σ) for x in -n:n]
...
function with_gaussian_blur(image; σ=1, l=1)
	
	g = [gauss(x,y, σ=σ) for x in -l:l,y in -l:l]
...
# ╔═╡ dbb59f3a-d4e6-4d35-88cb-ab7b21ce8cfd
with_gaussian_blur(philip_head,l=5)
...
function with_sobel_edge_detect(image)
	...
	Gx = [1 0 -1;2 0 -2;1 0 -1] .^ 2
	Gy = [1 2 1;0 0 0;-1 -2 -1] .^ 2
	
	Gt = (Gx+Gy) .^ 0.5
		
	# println(Gt)
	return convolve(image,Gt)
end

# ╔═╡ 81560674-ca03-42ca-9120-76ae5ebb8713
